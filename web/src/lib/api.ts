...
export type ContactIntakePayload = {
  ...
  company: string;
  use_case: string;
  source?: "launch-page" | "sales-demo" | "partner-referral";
};

export interface ContactIntakeRecord extends ContactIntakePayload {
  id: string;
  created_at: string;
  status: "received";
}
...
export const api = {
  ...
  submitContactIntake: (payload: ContactIntakePayload) =>
    fetchJSON<ContactIntakeSubmission>("/api/contact-intakes", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
